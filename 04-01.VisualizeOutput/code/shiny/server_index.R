server <- function(input, output, session) {
  ...
  # Cargar datos                            ####
  #df <- readRDS('N:/UDMTD/UDTMDCOM/AdvITI_github/04-01.VisualizeOutput/data/E30052/AdvITI_ts_indices.rds')
  df <- readRDS('AdvITI_ts_indices_publi.rds')
    ...
    output$plot <- renderPlot({
      ...
      batch_labels <- c('batch 01: t+20', 'batch 02: t+27', 'batch 03: t+38');
      names(batch_labels) <- batch_values
      ggplot(data = tabla(), aes(date, index)) + 
        ...
        geom_errorbar(aes(ymin=index - rmse, ymax= index + rmse), width= 14, colour = 'grey17', position= position_dodge(.9)) +
        ...
        labs(title = 'Industrial Turnover Index', x = '', y= '', colour = 'Index Version') +
        theme_bw() +
