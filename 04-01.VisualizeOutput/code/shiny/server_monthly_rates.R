server <- function(input, output, session) {
  ...
  # Cargar datos                            ####
  #df <- readRDS('N:/UDMTD/UDTMDCOM/AdvITI_github/04-01.VisualizeOutput/data/E30052/AdvITI_ts_monthlyRates.rds')
  df <- readRDS('AdvITI_ts_monthlyRates_publi.rds')
    ...
    output$plot <- renderPlot({
      ...
      ggplot(data = tabla(), aes(date, monthly_rate)) + 
        ...
        geom_errorbar(aes(ymin= monthly_rate - rmse, ymax= monthly_rate + rmse), width= 14, colour = 'grey17', position= position_dodge(.9)) +
        ...
        labs(title = 'Industrial Turnover Index Monthly Rate', x = '', y= '', colour = 'Index Version') +
        theme_bw() +
