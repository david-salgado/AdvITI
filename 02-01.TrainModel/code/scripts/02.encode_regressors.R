...
inputFD_fileName <- xml_text(xml_find_all(paramLocal.xml, "//files_info//historical_FD_woNA_fileName"))
inputFD_fullFileName <- file.path(path_data_intermediate_survey, inputFD_fileName)

outputFD_fileName <- xml_text(xml_find_all(paramLocal.xml, "//files_info//encoded_historical_FD_fileName"))
outputFD_fullFileName <- file.path(path_data_survey, outputFD_fileName)
...
#####                          :::::::::::::::                              #### 
#####                           CREATE LOG                                  ####
logFile <- create_log(path_log_survey, name_logFile = "log_encoding_regressors", sink_log = TRUE)
...
woNA_encoded_FD.dt <- encode_regressors(dt)

#####               Check for duplicates                                    ####

dupli.dt <- woNA_encoded_FD.dt[duplicated(woNA_encoded_FD.dt, by = id_var)]
if(nrow(dupli.dt) > 0){
...
saveRDS(woNA_encoded_FD.dt, outputFD_fullFileName)
